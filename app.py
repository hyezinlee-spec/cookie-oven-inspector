...
def check_visual_ai(image, res_type):
    ...
    **🔵 가독성 :** (텍스트와 이미지소스가 겹쳐보이니 가독성을 위해 수정 바랍니다 / 문제 되지 않습니다)  
    ...
    [기준] 목업: {mockup_instruction}, 저작권: 실사/제품 이미지 체크, 가독성: 텍스트 겹침/깨짐 체크.
    """
    
    # --- 핵심: 에러 발생 시 조용히 넘어가기 위한 try-except ---
    try:
...
st.title("🍪 쿠키오븐 제작가이드 통합 검수 (v5.9)")
...
if file:
    ...
    with col2:
        ...
        # AI 결과 처리 (에러 문구가 아닐 때만 목업 체크)
        if "의심됩니다" in ai_opinion and "AI 검수 사용량" not in ai_opinion and res_type != "참여중 영역":
